...
function supabaseStore({ url, serviceKey }) {
  ...
  return {
    ...
    async demographics({ page_id, breakdown, limit = 40 }) {
      const parts = ['select=*'];
      if (page_id) parts.push(`page_id=eq.${encodeURIComponent(page_id)}`);
      if (breakdown) parts.push(`breakdown=eq.${encodeURIComponent(breakdown)}`);
      parts.push('order=value.desc.nullslast');
      parts.push(`limit=${Math.min(Number(limit) || 40, 500)}`);
      return read('meta_page_demographics', parts.join('&'));
    },
