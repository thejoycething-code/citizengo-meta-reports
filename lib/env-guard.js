...
// Called by lib/tokens.js. TOKEN_STORE=off makes the connector ignore the
// per-person token table and trust MCP_TOKENS alone; the suites need that, the
// API in production must never have it.
function assertTokenStoreSafe() {
  if (String(process.env.TOKEN_STORE || '').toLowerCase() !== 'off') return;
  if (onVercelProduction()) {
    refuse('TOKEN_STORE=off', 'It would ignore every issued and revoked per-person token.');
  }
}

module.exports = {
  assertGraphHostSafe, assertDurableGuardSafe, assertGoogleEndpointsSafe, assertTokenStoreSafe,
  onVercelProduction, inGitHubActions,
};
