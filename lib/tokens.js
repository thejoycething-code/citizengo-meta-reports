...
//
// Tokens EXPIRE, 90 days by default (TOKEN_TTL_DAYS at issue time). A credential
// nobody withdraws and nobody re-confirms outlives the reason it was issued.
// Expiry is checked here rather than left to the query, so the rule holds even
// if a row is written by hand or the filter is ever loosened.
//
  ...
  cache.pending = (async () => {
    try {
      const res = await fetch(
        `${s.base}/rest/v1/meta_access_tokens?select=name,token_hash,expires_at&revoked_at=is.null&limit=1000`,
        { headers: s.headers },
      );
...
// A row is live when it is not revoked and not past its expiry. Rows written
// before the expiry column existed carry null and are treated as live; the
// migration backfilled every one of them, so null means "issued by hand".
function live(row) {
...
async function identifyStored(supplied) {
  ...
  let matched = null;
  for (const r of (await activeRows()).filter(live)) {
...
async function nameActive(name) {
  ...
  return (await activeRows()).filter(live).some((r) => r.name === name);
}

function resetCache() { cache = { rows: null, at: 0, pending: null }; touched.clear(); }

module.exports = { identify, identifyStored, nameActive, activeRows, live, hash, resetCache, CACHE_MS };
