...
function teamTokenIdentity(supplied) {
  // Weak tokens are dropped by usableTokens and never authenticate; the
  // comparison is over digests with timingSafeEqual. See lib/guard.js.
  const guard = require('./guard');
  return guard.identify(supplied, guard.usableTokens(process.env.MCP_TOKENS));
}

function teamTokenValid(supplied) {
  return teamTokenIdentity(supplied) !== null;
}
...
module.exports = {
  ...
  resourceOf, claimsFor, resourceMatches,
};
